...
def draw(image, boxes, scores, classes):
    for box, score, cl in zip(boxes, scores, classes):
        ...
        w *= image.shape[1]
        h *= image.shape[0]
        print(f"[turret contoller] Detection Class: {CLASSES[cl]} and Score: {score:.0%}")
        print(f"[turret contoller] Detection box (left,top,right,down): [{(x):.1f}, {(y):.1f}, {(x+w):.1f}, {(y+h):.1f}]")
...
def turret_controller(queue, event, modelparams, yaw_motor, pitch_motor, model_location, shared_lib_location, video_device):
    ...
    while not event.is_set():
        ...
        if ret:
            ...
            data = res_data.copy()
            del res_data
            if DEBUG: print(f"[turret contoller] Inference time : {(end - start):.5f} seconds")
            ...
            result_data = [img, boxes, scores, classes]
            
            if queue.full():
                #if the queue is full then take the oldest one out as its a stale frame
                queue.get()
